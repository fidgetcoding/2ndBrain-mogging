...
When a vault sync commits `/autoresearch` output, the commit message MUST start with `[bot:autoresearch]`. This matches the `05-Tasks/` sync filter and prevents W1 from re-ingesting research runs as tasks.
